...
const LandingPage = () => {
  ...
  const API = "http://127.0.0.1:5000";
  const nodeAPI = "http://localhost:3000"; // Ensure this points to your backend
  ...
  const fetchSavedCourses = async () => {
    ...
    try {
      ...
      const data = await response.json();
      console.log("Fetch Saved Courses Response:", data);
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-grow mt-28 px-6">
        <div className="bg-white rounded-lg p-6 shadow-lg">
          ...
          {activeTab === "courses" && (
            <>
              ...
              {searchTerm !== "" && (
                <p className="text-center mt-4 mb-6 font-semibold text-lg">
                  Search results for{" "}
                  <span className="text-green-500">"{searchTerm}"</span>
                </p>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-gray-100 p-4 rounded-lg">
                <Course
                  courses={
                    searchResults.length > 0 ? searchResults : topCourses
                  }
                  onSave={saveCourse}
                  onRecommend={fetchRecommendedCourses}
                />
          ...
          {activeTab === "saved" && !currentUser && (
            <p>Please log in to view your saved courses.</p>
          )}
        </div>
      </div>
